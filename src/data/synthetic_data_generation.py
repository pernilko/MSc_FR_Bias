...
#     G, img, label, global_blur_val=None, mask_blur_val=None, return_msk=False
# ):
#     print("num classes: ", G.attr_map.fc0.init_args[0])
#     print("label: ", torch.tensor(label))
#     ohe_label = torch.nn.functional.one_hot(
#         torch.tensor(label), num_classes=G.attr_map.fc0.init_args[0]
#     ).to(img.device)
...
#     a_out = G.attr_map(
#         ohe_label.to(s_out.device), None, truncation_psi, truncation_cutoff
#     )
...
#     if return_msk:
#         to_return = (
#             (img_out, msk, cam) if G.learn_mask is not None else (img_out, None, None)
#         )
#     else:
#         to_return = img_out
...
#         img = np.array(
#             PIL.Image.open(file).resize((side, side)), dtype=np.float32
#         ).transpose((2, 0, 1))
#         images.append(img)
...
#     images_as_tensor_exp = (
#         images_as_tensor[:, None]
#         .expand([number_of_images, aging_steps, *images_as_tensor.shape[1:]])
#         .reshape([-1, *images_as_tensor.shape[1:]])
#     )

#     labels_exp = torch.tensor(
#         np.repeat(
#             np.linspace(*data_labels, aging_steps, dtype=int)[:, None],
#             number_of_images,
#             1,
#         ).T.reshape(-1)
#     )
...
#                 images_as_tensor_exp.split(batch_size), labels_exp.split(batch_size)
#             )
#         ]
#     )
...
# def plot_output(
#     batch_of_filenames, img_in_tensor, img_out_tensor, labels_exp, aging_steps
# ):
...
#             1, aging_steps + 1, figsize=(aging_steps * 4, 4), dpi=100
#         )
...
#     configs = {
#         FFHQ_LS_KEY: dict(
#             gdrive_id="1sWSH3tHgm9DkHrc19hoEMrR-KQgnaFuw", side=256, classes=(1, 8)
#         ),
#         FFHQ_RR_KEY: dict(
#             gdrive_id="17BOTEa6z3r6JFVs1KDutDxWEkTWbzaeD", side=224, classes=(20, 65)
#         ),
#     }
...
#     out_tensor, images_as_tensor, labels_exp = prep_data(
#         side_config, batch_of_filenames, data_labels_range, g_ema
#     )
